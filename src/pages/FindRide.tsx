...
function FindRide(props: any) {
  ...
  const [paymentMode, setPaymentMode] = useState(""); // default is wallet
  ...
  const getPaymentAmount = (distance: number) => {
    ...
    let total = 25 + chargeableDistance * ratePerKm;
  ...
  useEffect(() => {
    ...
    const checkIsOnRide = async () => {
      let result = await axios.get(
        "https://ridesyncbackend.onrender.com/get_is_on_ride",
        {
          params: {
            userId: userId,
          },
  ...
  <IonText className="label">Select Payment Mode:</IonText>
  <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
    <IonButton
      fill={paymentMode === "wallet" ? "solid" : "outline"}
    ...
    <IonButton
      fill={paymentMode === "cash" ? "solid" : "outline"}
    ...
    >
      Cash
              ...
              <IonButton
  disabled={paymentMode === "wallet" && paymentAmount > balance}
  onClick={handleSearchClick}
>
  {paymentMode === "wallet" && paymentAmount > balance
    ...
    <div className="ridesHolder searchRidesModalContent">
      ...
      {props.isMessageVisible && (
        <div className="requestedMessage">
          <IonText>
            You have requested to join ride(s).
          </IonText>
        </div>
      )}
      <div className="amountMessage">
  <IonText>
    {props.paymentMode === "wallet" ? (
      <>
      ...
      <>Please pay ₹ {props.paymentAmount} in cash after your ride.</>
    )}
  </IonText>
</div>
      {props.rides &&
          ...
          <>
            <div className="rideCard" key={ride?.Id}>
              ...
              <div className="rideCardFooter">
                ...
                {/* ✅ Start Time shown below everything in one line */}
                {ride?.StartTime && (
                  <div style={{ marginTop: "8px", textAlign: "center" }}>
                  <IonText>
                      Start Time:{" "}
                  <strong> {new Date(ride.StartTime).toLocaleString("en-IN", {
                                      dateStyle: "medium",
                                      timeStyle: "short",
                            })}
                  </strong>
