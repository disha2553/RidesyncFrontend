...
function RideDetails(props: any) {
  ...
  useEffect(() => {
    if (corider?.PickupTime) {
      const pickupTime = new Date(corider.PickupTime).getTime();
      const cancelDeadline = pickupTime + 5 * 60 * 1000; // 5 minutes after pickup
      const now = new Date().getTime();
      const remaining = Math.floor((cancelDeadline - now) / 1000);
  ...
  useEffect(() => {
    ...
    setCoRiders(cds);
    const joinedCount = rideDetails?.CoRiders?.filter(
      (r: any) => r.Status === "Joined"
    )?.length || 0;
    setCoRidersJoined(joinedCount);
  ...
  useEffect(() => {
    if (rideDetails?.StartTime) {
        const rideStartTime = new Date(rideDetails.StartTime.seconds * 1000);
        const timer = setTimeout(() => setShowCodeInput(true), 5 * 60 * 1000); // 5 min timer
  ...
  const onRiderCompleteClick = async (coriderId: string, index: number) => {
    ...
    try {
      let data = await axios.get(
        "https://ridesyncbackend.onrender.com/complete_corider_ride",
        {
          params: {
            rideId: rideId,
            coriderId: coriderId,
  ...
  const onRiderCompleteCashClick = async (coriderId: string) => {
    setIsLoadingOpen(true);
    try {
      let data = await axios.get("https://ridesyncbackend.onrender.com/complete_corider_cash", {
        params: {
          rideId: rideId,
          coriderId: coriderId,
        },
  ...
  const onDirectCancelClick = async (coriderId: string) => {
    setIsLoadingOpen(true);
  
    try {
      let data = await axios.get("https://ridesyncbackend.onrender.com/direct_cancel_corider_ride", {
        params: {
          rideId: rideId,
          coriderId: coriderId,
        },
      ...
      setIsLoadingOpen(false);
      setHeader("Error");
      setMessage(e.response ? e.response.data.error : "Unknown error");
      setIsAlertOpen(true);
    }
  };

  
  const onRiderCancelClick = async (coriderId: string, index: number) => {
    setIsLoadingOpen(true);

    // Get the cancellation code from the input field
    let input = document.getElementById(`cancelInput${index}`) as HTMLInputElement;
    let cancellationCode = parseInt(input.value);

    if (!cancellationCode) {
        setIsLoadingOpen(false);
        setHeader("Error");
        setMessage("Please enter a valid cancellation code.");
        setIsAlertOpen(true);
        return;
    }

    try {
        let data = await axios.get("https://ridesyncbackend.onrender.com/cancel_corider_ride", {
            params: {
                rideId: rideId,
                coriderId: coriderId,
                cancellationCode: cancellationCode, // Corrected variable name
            },
        });
  ...
  const handleCompleteDriverRideClick = async () => {
    ...
    try {
      let data = await axios.get(
        "https://ridesyncbackend.onrender.com/complete_driver_ride",
        {
          params: {
            rideId: rideId,
          },
      ...
      console.log(data.data);
      const { walletEarnings, cashEarnings } = data.data;
      setHeader("Ride Completed");
      setMessage(
        `You earned ₹${walletEarnings} in wallet.\n₹${cashEarnings} was collected in cash from riders.`
      );
      setIsAlertOpen(true);
    ...
    <>
      <IonPage>
                  ...
                  >
                    <div className="userDetails">
                      ...
                      <div className="userName">
                        ...
                        {rideDetails?.StartTime && (
                            ...
                            <IonText>
                                ...
                                <span style={{ color: "green" }}>
                                  {new Date(rideDetails?.StartTime).toLocaleString("en-IN", {
                                    dateStyle: "medium",
                                    timeStyle: "short",
                                  })}
                                </span>
                        ...
                        {role == "driver" && (
                        <IonText >
                          Ride Cancellation Code: <span style={{ color: "green" }}>{rideDetails?.CancellationCode}</span>
                        </IonText>
                        )}
                      </div>
                      {role === "corider" && corider?.Status !== "Completed" && corider?.Status !== "Cancelled" &&(
                        <IonButtons>
                          <IonButton
                ...
                rideDetails?.Status === "Completed" ? (
                  ...
                  rideDetails?.Status === "Completed" ? (
                                ...
                                <div className="amountHolder">
                                  {role == "driver" && (
                                    <>
                                      <IonText>
                                        {rideDetails?.Status === "Completed"
                                          ? corider ?.PaymentMode === "wallet"
                                            ? "You got paid through wallet"
                                            : "You got paid in cash"
                                          : corider ?.PaymentMode === "wallet"
                                            ? "You'll get paid via wallet"
                                      ...
                                      <>
                                        <IonText>
                                            {corider?.Status === "Completed"
                                              ? corider?.PaymentMode === "wallet"
                                                ? "You paid through wallet"
                                                : "You paid in cash"
                                              : corider?.PaymentMode === "wallet"
                                                ? "You will pay through wallet"
                                    ...
                                    {corider?.Status === "Completed" ? (
                                      <>
                                        ...
                                        <IonText>
                                          ...
                                          ></IonIcon>
                                          Drop Time: {new Date(corider?.DropTime).toLocaleString("en-IN", {
                                      dateStyle: "medium",
                                      timeStyle: "short",
                            })}
                                        </IonText>
                                      ...
                                      <>
                                        {role == "driver" && (
                                          ...
                                          {corider?.PaymentMode === "wallet" ? (
                                            <>
                                            <IonText> Ask co-rider for ride completion code to complete this ride. </IonText>
                                            <div className="actionButtons">
                                              <input id={"input" + index} type="tel" placeholder="Code" maxLength={5} />
                                                <IonButton onClick={() => onRiderCompleteClick(corider?.Id, index )} > 
                                                    Complete
                                                </IonButton>
                                            </div>
                                            </>
                                            ...
                                            <>
                                            <IonText> Once you receive the cash payment from co-rider, click on Complete. </IonText>
                                            <div className="actionButtons" style={{ display: "flex", justifyContent: "center", marginTop: "10px" }}>
                                              <IonButton onClick={() => onRiderCompleteCashClick(corider?.Id)} >
                                                    Complete
                                              </IonButton>
                                            </div>
                                            </>
                                        ...
                                        {role == "corider" &&
                                          corider?.CoRider?.Id == userId && (
                                            <>
                                              {corider?.PaymentMode === "wallet" ? (
                                                <>
                                                  <IonText>Your ride completion code is.</IonText>
                                                  <IonText
                                                    style={{
                                                      fontSize: "25px",
                                                      margin: "10px 0",
                                                  ...
                                                  <IonText style={{ color: "darkgray" }}>
                                                    Share with the driver when you have reached your drop location.
                                                  </IonText>
                                                </>
                                              ) : (
                                                <>
                                                  <IonText>
                                                    You have to pay ₹{corider?.Amount} in cash when you reach your
                                                    destination.
                                                  </IonText>
                                                </>
                                              )}
                                              ...
                                              {secondsLeft !== null && secondsLeft > 0 ? (
                                                ...
                                                <IonText style={{ display: "block", marginBottom: "10px" }}>
                                                    You can cancel directly for next <b>{formatTime(secondsLeft)}</b>
                                                </IonText>
                                                <div className="actionButtons" style={{ display: "flex", justifyContent: "center", marginTop: "10px" }}>
                                                <IonButton color="danger" onClick={() => onDirectCancelClick(corider?.Id)}>
                                                    Cancel
                                                </IonButton>
                                                </div>
                                                </>
                                                ...
                                                <>
                                                <IonText style={{ marginTop: "15px", display: "block" }}>
                                                    If you want to cancel your ride, ask the driver for the cancellation code.
                                                </IonText>
                                                <div className="actionButtons">
                                                  <input id={"cancelInput" + index} type="tel" placeholder="Code" maxLength={5}
                                                  />
                                                  <IonButton color="danger" onClick={() => onRiderCancelClick(corider?.Id, index)} >
                                                      Cancel
                                                  </IonButton>
                                                </div>
                                                </>
                                  ...
                                  >
                                    <div className="userDetails">
                                      ...
                                      <div className="userName">
                                        ...
                                        <br />
                                        <IonText>
                                          {request?.CoRider?.Gender}
                                        </IonText>
                                        <br />
                                        <IonText style={{ color: 'gray' }} >Tap to show details</IonText>
                                      </div>
                                    </div>
                                  </div>
                                  <div className="dashedLine"></div>
                                  <div className="amountHolder">
                                    <IonText>You'll get paid {request?.PaymentMode === "wallet" ? "via Wallet" : "in Cash"} </IonText>
                    ...
                    <div className="infoHolder">
                      ...
                      >
                        {rideDetails?.Status === "Started" && role == "driver" &&
        ...
        {((role == "driver" && rideDetails?.Status === "Completed") ||
          ...
          >
            <IonIcon icon={checkmarkCircleOutline} color="light"></IonIcon>
            <IonText style={{ marginLeft: "10px", color: "white" }}>Completed</IonText>
          </IonFooter>
        )}
